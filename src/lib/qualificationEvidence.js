...
export function computeQualificationEvidenceSummary({ qualification = {}, certificates = [], today = null }) {
  ...
  if (requirementMode === 'base_refresh') {
    ...
    const refreshValidityMonths = Number.isFinite(qualification?.certificate_refresh_validity_months)
      ...
      : null;
    const bases = normalizedCertificates.filter((certificate) => ['base', 'recertification', 'single'].includes(certificate.evidence_role));
    const refreshes = normalizedCertificates
      .filter((certificate) => certificate.evidence_role === 'refresh')
      .sort((left, right) => (deriveEffectiveFrom(left) || '').localeCompare(deriveEffectiveFrom(right) || ''));
    ...
    if (!bases.length) {
      return {
        ...
        missing_roles: ['base'],
        active_certificate_ids: refreshes.map((certificate) => certificate.id),
        certificate_valid_until_by_id: {},
      };
    }
  ...
  const winner = [...normalizedCertificates].sort((left, right) => {
    ...
    if (leftUntil !== rightUntil) return rightUntil.localeCompare(leftUntil);
    return (deriveEffectiveFrom(right) || '').localeCompare(deriveEffectiveFrom(left) || '');
  })[0];
